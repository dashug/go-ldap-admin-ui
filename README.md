...
本仓库基于原作者 [eryajf/go-ldap-admin-ui](https://github.com/eryajf/go-ldap-admin-ui) 二次开发，感谢原作者与所有贡献者的开源付出。
...
本分支聚焦“可落地、低门槛”的配置体验，配合后端增强实现了新手可操作的向导式接入流程。

## 🚀 本分支升级清单

### 配置体验升级（面向小白）

- 新增“目录快速配置”入口：可视化配置 OpenLDAP/AD 参数并保存。
- 新增“平台对接向导”：按平台分步配置钉钉、企微、飞书。
- 新增“测试连接”交互：保存前可先验证凭证是否可用。

### 对接效率升级

- 用户管理页支持直接查看当前目录类型（OpenLDAP / AD）。
- 分组管理页增加快捷入口，可一键跳转到配置入口。
- 配置表单支持预填当前已保存参数，减少重复录入。
